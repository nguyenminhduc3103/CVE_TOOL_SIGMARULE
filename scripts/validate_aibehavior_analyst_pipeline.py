...
async def validate(cve_id: str):
    ...
    print(f"[*] Đang chạy TriageOrchestrator để gọi AI phân tích {cve_id}...")
    ...
    if not tech_analysis and not attack_mapping:
        print("[-] AI phân tích thất bại hoặc không có kết quả TechnicalAnalysis/AttackMapping.")
        print(f"    Triage decision: {enriched.triage.decision} — {enriched.triage.decision_reason}")
        sys.exit(1)

    print("[+] Đã nhận kết quả phân tích từ AI. Bắt đầu đối chiếu Ground Truth...")

    print("\n[MÃ JSON MÀ AI ĐÃ SINH RA]")
    if tech_analysis:
