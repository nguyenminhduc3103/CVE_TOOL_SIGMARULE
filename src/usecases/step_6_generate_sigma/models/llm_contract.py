"""Step 6 LLM contract — strict schema for AI Detection Logic Planner output.

AI emits ONLY:
    detections[].intent (free text)
    detections[].priority ∈ {critical, high, medium, low}
    detections[].rationale
    detections[].selection_hint (optional, evidence-backed only)
    logic.operator ∈ {all, any, at_least}
    logic.operands (list of detection-plan indexes)
    logic.threshold (required when operator=at_least)
    falsepositives (list of strings)
    risk_bias ∈ {conservative, neutral, aggressive}
    rationale (string)
    planner_confidence (0.0-1.0)

AI must NOT emit:
    - domain / telemetry category (Step 4 owns that)
    - logsource (Step 4 sigma_logsources is canonical)
    - Sigma field names / modifiers (Step 4 KB owns taxonomy)
    - condition literal string (Builder renders)
    - level (Builder computes)
    - title / id / status / author / date / tags / references
    - UUID, YAML

`Step6LLMResponse` is the strict raw-LLM schema. The post-processed domain
model is `DetectionPlan` in `domain/detection_plan.py`.
"""
from __future__ import annotations

from typing import Literal

from pydantic import BaseModel, Field, model_validator


Priority = Literal["critical", "high", "medium", "low"]
Operator = Literal["all", "any", "at_least"]
RiskBias = Literal["conservative", "neutral", "aggressive"]


_FORBIDDEN_LLM_FIELDS = frozenset({
    "domain", "telemetry_domain",
    "logsource", "category", "product", "service",
    "level",
    "title", "id", "status", "author", "date", "tags", "references",
    "condition",
    "uuid",
    "yaml",
})


class RawDetectionIntent(BaseModel):
    intent: str = Field(min_length=1)
    priority: Priority = "medium"
    rationale: str = ""
    selection_hint: dict[str, list[str]] | None = None

    @model_validator(mode="before")
    @classmethod
    def _coerce_selection_hint(cls, data: object) -> object:
        if not isinstance(data, dict):
            return data
        hint = data.get("selection_hint")
        if not isinstance(hint, dict):
            return data
        coerced: dict[str, list[str]] = {}
        for k, v in hint.items():
            if isinstance(v, list):
                coerced[k] = [str(x) for x in v]
            elif v is None:
                # Skip null entries; equivalent to omitting the field.
                continue
            elif isinstance(v, (str, int, float, bool)):
                coerced[k] = [str(v)]
            else:
                # Unknown shape — coerce via str() to keep the contract valid.
                coerced[k] = [str(v)]
        data["selection_hint"] = coerced or None
        return data


class RawDetectionLogic(BaseModel):
    operator: Operator
    operands: list[int] = Field(min_length=1)
    threshold: int | None = None

    @model_validator(mode="after")
    def _validate_logic(self) -> "RawDetectionLogic":
        if self.operator == "at_least":
            if self.threshold is None or self.threshold < 1:
                raise ValueError("operator=at_least requires threshold >= 1")
            if self.threshold > len(self.operands):
                raise ValueError("threshold cannot exceed operand count")
        for idx in self.operands:
            if idx < 0:
                raise ValueError("operands must be non-negative detection-plan indexes")
        return self


class Step6LLMResponse(BaseModel):
    """Strict schema for AI Detection Logic Planner output."""

    detections: list[RawDetectionIntent]
    logic: RawDetectionLogic
    falsepositives: list[str] = Field(default_factory=list)
    risk_bias: RiskBias = "neutral"
    rationale: str = ""
    planner_confidence: float = Field(ge=0.0, le=1.0, default=0.0)

    @model_validator(mode="after")
    def _validate_plan(self) -> "Step6LLMResponse":
        if not self.detections:
            raise ValueError("detections must be non-empty")
        max_idx = len(self.detections) - 1
        for idx in self.logic.operands:
            if idx > max_idx:
                raise ValueError(
                    f"logic operand {idx} out of range "
                    f"(plan has {len(self.detections)} intents)"
                )
        return self


def assert_no_forbidden_fields(raw_dict: dict) -> None:
    """Defensive guard: reject LLM responses carrying forbidden fields.

    Pydantic's strict schema drops unknown fields by default, but a raw dict
    could still smuggle them. Use this when receiving AI output to fail loud
    rather than silently dropping.
    """
    present = set(raw_dict.keys()) & _FORBIDDEN_LLM_FIELDS
    if present:
        raise ValueError(
            f"AI response contains forbidden fields: {sorted(present)}. "
            f"AI must only emit: detections, logic, falsepositives, risk_bias, "
            f"rationale, planner_confidence."
        )


__all__ = [
    "Priority",
    "Operator",
    "RiskBias",
    "RawDetectionIntent",
    "RawDetectionLogic",
    "Step6LLMResponse",
    "assert_no_forbidden_fields",
]