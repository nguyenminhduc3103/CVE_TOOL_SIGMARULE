...
class OpenCTIProvider(BaseProvider):
    ...
    async def enrich(self, cve_id: str) -> CoreCVEData | None:
        """Làm giàu dữ liệu lỗ hổng bằng cách tìm kiếm trong collection OpenCTI."""
        self.logger.info("[OpenCTI] Đang truy vấn collection để làm giàu mã CVE", cve_id=cve_id)
        try:
            ...
            for cve in parsed_cves:
                if cve.cve_id.upper() == cve_id.upper():
                    self.logger.info("[OpenCTI] Tìm thấy CVE trùng khớp", cve_id=cve_id)
                    return cve
            self.logger.warning("[OpenCTI] Không tìm thấy CVE trong lô dữ liệu", cve_id=cve_id)
            return None
        except Exception as exc:
            self.logger.warning(
                "[OpenCTI] Lỗi khi làm giàu dữ liệu lỗ hổng",
                cve_id=cve_id,
                error=str(exc)
            )
            return None
    ...
    async def fetch(self, cve_id: str) -> Any:
        """Thu thập và trả về dữ liệu đã được chuẩn hóa của một mã CVE ID cụ thể dưới dạng dictionary JSON."""
