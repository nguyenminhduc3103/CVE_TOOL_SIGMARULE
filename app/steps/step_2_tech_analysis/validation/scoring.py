...
def compute_verdict(
    ...
    # Đảm bảo technique_match_rate để xét điều kiện không bị lỗi NoneType (gán = 0 nếu None)
    eff_rate = technique_match_rate if technique_match_rate is not None else 0.0

    # 2. Xử lý PASS: Điểm kỹ thuật cao (>=70%) và Rủi ro FP không phải mức HIGH
    if eff_rate >= _PASS_THRESHOLD and fp_risk in ("LOW", "MEDIUM"):
        reason = (
            f"AI techniques match {eff_rate:.0%} of expected ground truth "
    ...
    if fp_risk == "HIGH":
        reason = (
            ...
            f"(technique_match_rate={eff_rate:.0%})."
        )
        return overall, "FAIL", reason

    # 4. Xử lý PARTIAL: Điểm kỹ thuật ở ngưỡng chấp nhận được (>= 40% nhưng chưa được 70%)
    if eff_rate >= _PARTIAL_THRESHOLD:
        reason = (
            f"Partial match: AI covers {eff_rate:.0%} of expected techniques "
            f"(threshold for PASS is {_PASS_THRESHOLD:.0%}). "
            f"FP risk: {fp_risk}."
        )
        return overall, "PARTIAL", reason

    # 5. Mặc định là FAIL: Điểm kỹ thuật dưới chuẩn (< 40%)
    reason = (
        f"Low technique match rate ({eff_rate:.0%} < {_PARTIAL_THRESHOLD:.0%}). "
        f"AI may have missed critical attack techniques for this CVE."
    )
    return overall, "FAIL", reason
