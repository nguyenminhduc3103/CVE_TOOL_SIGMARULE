...
# "none"/"unknown" placeholders AI hay trả về.
...
def _validate_ttp_field(
    ...
    Returns:
        {
            "valid": bool,
            "valid_techniques": [...], "valid_tactics": [...], "valid_subtechniques": [...],
            "invalid_techniques": [...], "dropped_semantic": [...],
            "reason": str | None,
        }
    """
...
def validate_field_level(
    ...
    Mỗi field đánh giá độc lập: valid=True giữ nguyên, valid=False set None + reason.

    Returns:
        {
            "valid": bool,
            "invalid_fields": {field_path: reason, ...},
            "validated_data": dict (invalid field đã set None, valid đã clean),
            "ttp_validation": {...},
        }
    """
    ...
    mb = _validate_list_field(tech.get("mandatory_behaviors"), "mandatory_behaviors")
    if not mb["valid"]:
        invalid_fields["technical_analysis.mandatory_behaviors"] = mb["reason"]

    # evasive_indicators: mandatory cho memory-corruption HOẶC code-injection CWE
    # (CWE-787/125/416/119/190 hoặc CWE-94/95/96/917/1336). Empty list → invalid
    # → trigger partial-fill retry.
    ...
    # subtechniques rỗng sau whitelist → fill ["none"] sentinel để downstream
    # phân biệt "đã cố, không tìm được sub" với "chưa chạy pipeline".
...
def _apply_partial_fill(base: dict[str, Any], fill: dict[str, Any], invalid_paths: dict[str, str]) -> dict[str, Any]:
    ...
    Field valid trong base → giữ nguyên, KHÔNG động vào.
    Field invalid → lấy từ fill (nếu fill có).

    KHÔNG merge dict-style (Union/Replace). Chỉ pointwise replacement theo path.
    Đây là fix cho wipeout bug CVE-2023-22515.
    """
...
def _deep_copy_dict(d: dict[str, Any]) -> dict[str, Any]:
    """Shallow copy dict layer. Value (list/str/int) dùng reference — OK vì
