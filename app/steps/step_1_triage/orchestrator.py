...
def _safe_get_first_line(exc: Any) -> str:
    if not exc:
        return "Unknown error"
    text = str(exc)
    return next(iter(text.splitlines()), type(exc).__name__ if isinstance(exc, Exception) else "Unknown error")
...
class TriageOrchestrator:
    ...
    async def _run_stage(
        ...
        try:
            ...
            self.logger.warning("[ORCHESTRATOR] stage_failed", stage=stage_name, cve_id=cve_id, error=_safe_get_first_line(exc))
            return fallback, True
    ...
    async def _run_analysis_stage(self, context: EnrichedCVEContext, capability):
        ...
        if getattr(settings, "ai_enabled", False):
            ...
            try:
                ...
                self.logger.warning(
                    ...
                    cve_id=context.core.cve_id,
                    error=_safe_get_first_line(exc),
                )
        ...
        try:
            ...
            self.logger.warning("[ORCHESTRATOR] stage_failed", stage="analysis_stage", cve_id=context.core.cve_id, error=_safe_get_first_line(exc))
    ...
    async def _run_enriched_stage(self, stage_name: str, stage_fn, context: EnrichedCVEContext, capability, fallback):
        try:
            ...
            self.logger.warning("[ORCHESTRATOR] stage_failed", stage=stage_name, cve_id=context.core.cve_id, error=_safe_get_first_line(exc))
            return fallback, True
    ...
    async def _run_provider(
        ...
        try:
            ...
            provider_errors[provider_name] = _safe_get_first_line(exc)
