...
class EPSSHTTPClient(BaseHTTPClient):
    ...
    async def fetch_raw(self, cve_id: str) -> Any:
        # TODO: add caching and rate-limit awareness if the API requires it.
        params = {"cve": cve_id}
        last_error: Exception | None = None

        for attempt in range(1, self.retries + 1):
            try:
                self.logger.info("[EPSS] Fetching CVE", cve_id=cve_id, attempt=attempt)
                response = await self.get(self.ENDPOINT, params=params)
                response.raise_for_status()
                payload = response.json()
                self.logger.info("[EPSS] Response received", cve_id=cve_id, status_code=response.status_code)
                return payload
